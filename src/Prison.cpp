...
void PrisonMapGen::GenRoomPos()
{
	...
	for (MapUnitData* roomData : _rooms)
	{
		...
		int tmpx = Random::random_int(0, 100),
			tmpy = Random::random_int(-12, 12);
...
void PrisonMapGen::Generate()
{
    ...
    while (true)
    {
        ...
        // 确定房间组成
        int AllNum = Random::random_int(12, 15),
            eliteNum = Random::random_int(0, 2),
            treasureNum = Random::random_int(1, 2),
            traderNum = 1,
            exitNum = 1;
        int specialNum = eliteNum + treasureNum + traderNum + exitNum,
            tripleNum = Random::random_int(1, (specialNum - 1) / 2),
        ...
        while (!roomQueue.empty())
        {
            ...
            int chosenSonNum = sonNumPool[Random::random_int(0, int(sonNumPool.size() - 1))];
            --sonNum[chosenSonNum];
            ...
            if (!chosenSonNum)
            {
                ...
                {
                    int chosenSpecialRoomNum = specialRoomPool[Random::random_int(0, int(specialRoomPool.size() - 1))];
            ...
            else
            {
                if (room != startRoomData)
                {
                    ...
                    int combatRoomIndex = Random::random_int(1, 60);
        ...
        this->timeTreeGen = duration<double, std::milli>(t1_end - t1_start).count();
        ...
        while (true)
        {
            ...
            this->timeForceLayout = duration<double, std::milli>(t2_end - t2_start).count();
            ...
            this->timeAStarCorridor = duration<double, std::milli>(t3_end - t3_start).count();
